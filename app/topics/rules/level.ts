...
export function goPrevious(args) {
    let subTopic: SubTopic = TopicService.getInstance().findSubTopicFromLink("topics/rules/crossing");
    navigationModule.gotoChapters(subTopic);
}

export function goNext(args) {
    let subTopic: SubTopic = TopicService.getInstance().findSubTopicFromLink("topics/rules/stopping");
    navigationModule.gotoChapters(subTopic);
}
