...
export function goPrevious(args) {
    let subTopic: SubTopic = TopicService.getInstance().findSubTopicFromLink("topics/rules/introduction");
    navigationModule.gotoChapters(subTopic);
}

export function goNext(args) {
    let subTopic: SubTopic = TopicService.getInstance().findSubTopicFromLink("topics/rules/lanes");
    navigationModule.gotoChapters(subTopic);
}
