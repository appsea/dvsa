...
export function goPrevious(args) {
    let subTopic: SubTopic = TopicService.getInstance().findSubTopicFromLink("topics/other/introduction");
    navigationModule.gotoChapters(subTopic);
}

export function goNext(args) {
    let subTopic: SubTopic = TopicService.getInstance().findSubTopicFromLink("topics/other/large");
    navigationModule.gotoChapters(subTopic);
}
